"""ASOS scraper — scrape ASOS search API, embed with local SigLIP, upsert Supabase."""
...
logger = logging.Logger("scraper-asos")
...
def run() -> dict[str, Any]:
    ...
    logger.info("=== ASOS scraper start (single process) ===")
    if not cfg.SUPABASE_URL or not cfg.SUPABASE_KEY:
        logger.error("SUPABASE_URL / SUPABASE_KEY missing")
        sys.exit(1)
    ...
    for k, v in summary.items():
        logger.info("%s: %s", k, v)
    logger.info("=== ASOS scraper complete ===")
    return summary


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ASOS scraper")
